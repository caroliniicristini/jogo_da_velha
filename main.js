let pound = {
    ...
    verificarGanhador(){
        return(
            pound.comparar(0, 1, 2) || pound.comparar(3, 4, 5) || pound.comparar(6, 7, 8) ||
            pound.comparar(0, 3, 6) || pound.comparar(1, 4, 7) || pound.comparar(2, 5, 8) ||
            pound.comparar(0, 4, 8) || pound.comparar(2, 4, 6)
...
function iniciar(){
    pound.newGame();
    while(!pound.jogoFinalizado) // vai rodar quando o jogo !não estiver iniciado
    {
        console.clear();
        pound.imprimir();
        console.log(`Jogador Atual: ${pound.jogadorAtual}`);
        const posicao = parseInt(prompt('Posição: ')) // vai pegar a string que o usuário escrever e converter em
        //numeros inteiros
        if(!posicao){
            break;
        }
